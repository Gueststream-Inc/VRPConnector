        ...
        <?php foreach ( $data as $index => $unit ) : ?>
            ...
            <div class="vrp-col-md-4 vrp-col-xs-12 vrp-col-sm-6 vrp-col-lg-4 vrp-item-wrap vrp-grid">
                <div class="vrp-item result-wrap"
                 ...
                 data-special-expiration="<?php echo date("m/d/Y", strtotime($unit->special->expiration)); ?>"
                 data-special-start_date="<?php echo date("m/d/Y", strtotime($unit->special->start_date)); ?>"
                 data-special-end_date="<?php echo date("m/d/Y", strtotime($unit->special->end_date)); ?>"
                 ...
                 >
                    <div class="vrp-result-image-container">
                        ...
                        style="background-image:url('<?php echo $unit->photos[0]->thumb_url; ?>');">
                        </a>
                    </div>
                        ...
                        <div class="vrp-results-wrap">
                            <h3>
                                <a  href="/vrp/unit/<?= $unit->page_slug; ?>"
                                    Title="<?php echo $unit->Name; ?>"><?php echo $unit->Name; ?> 
                                </a>
                            </h3>
                            <div class="vrp-result-line details"> <?= $unit->Bedrooms; ?> BR | <?= $unit->City; ?></div>

                            <div class="vrp-results-line rate"> 
                                <?php if (!empty($unit->Rate)) { ?>$<?php echo esc_html(number_format(($unit->Rate) / ($nights), 2)); ?>/night
                                <?php } else { ?>
                    ...
                    <div class="vrp-results-more">
                        <a  href="/vrp/unit/<?= $unit->page_slug; ?>" 
                            class="viewDetailsBtn">View
                            Unit
                        </a>
                    </div>
                </div> <!-- /vrp-item result-wrap -->
            </div><!-- /vrp-item-wrap vrp-grid -->
