...
function ratebreakdown(obj) {
    ...
    for (var i in obj.Charges) {
        if(obj.Charges[i].Amount>0) {
            var row = "<tr><td>" + obj.Charges[i].Description + "</td><td>$" + parseFloat(obj.Charges[i].Amount).toFixed(2) + "</td></tr>";
            tbl.append(row);
        }
    }
    if (obj.HasInsurance && obj.HasInsurance == 1) {
        var row = "<tr><td>Insurance</td><td>$" + parseFloat(obj.InsuranceAmount).toFixed(2) + "</td></tr>";
        tbl.append(row);
    }

    var totaldue = "<tr class='success'><td><b>Total Due Now USD:</b></td><td><b>$" + parseFloat(obj.DueToday).toFixed(2) + "</b></td></tr>";
    if(obj.TotalTax>0) {
        var tax = "<tr><td>Tax:</td><td>$" + parseFloat(obj.TotalTax).toFixed(2) + "</td></tr>";
        tbl.append(tax);
    }
    var total = '<tr class="double"><td><b>Trip Total:</b></td><td><b>$' +parseFloat(obj.TotalCost).toFixed(2) + '</b></td></tr>';
...
jQuery(document).ready(function () {
    ...
    //Unit Photo Gallery
        ...
        jQuery('.vrp-slider-nav').slick({
            slidesToShow: 3,
            slidesToScroll: 1,
            asNavFor: '#vrp-unit-slick',
            dots: false,
            centerMode: true,
            focusOnSelect: true,
            arrows: false,
