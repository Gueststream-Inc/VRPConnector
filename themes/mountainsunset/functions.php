...
class mountainsunset {
	...
	public function my_scripts_method() {
		wp_enqueue_script( 'google_maps_api', 'https://maps.googleapis.com/maps/api/js?v=3.exp' );

		$this->enqueue_local_script( 'vrp_jquery_ui', 'css/jquery-ui-1.11.2.custom/jquery-ui.js', [ 'jquery' ] );
		$this->enqueue_local_script( 'vrp_namespace', 'js/vrp.namespace.js', [ 'jquery' ] );
		$this->enqueue_local_script( 'vrp_mrespond_Module', 'js/vrp.mRespond.js', [ 'jquery' ] );
		$this->enqueue_local_script( 'vrp_ui_module', 'js/vrp.ui.js', [ 'jquery' ] );
		$this->enqueue_local_script( 'vrp_querystring_module', 'js/vrp.queryString.js', [ 'jquery' ] );
		$this->enqueue_local_script( 'vrp_theme_js', 'js/js.js', [ 'jquery' ] );
		$this->enqueue_local_script( 'vrp_result_map', 'js/vrp.resultListMap.js', [ 'jquery', 'google_maps_api' ] );
		$this->enqueue_local_script( 'vrp_unit_page', 'js/vrp.unit.js', [ 'jquery', 'google_maps_api' ] );
		$this->enqueue_local_script( 'vrp_submit_review', 'js/vrp.submitReview.js', [ 'jquery' ] );
		...
		wp_localize_script( 'vrp_theme_js', 'url_paths', $script_vars );

	}

	private function enqueue_local_script( $handle, $file, $dependencies = [] ) {

		if ( file_exists( get_stylesheet_directory() . '/vrp/' . $file ) ) {
			wp_register_script(
				$handle,
				get_stylesheet_directory_uri() . '/vrp/' . $file,
				$dependencies
			);
		} else {
			wp_register_script(
				$handle,
				plugins_url( '/mountainsunset/' . $file, dirname( __FILE__ ) ),
				$dependencies );
		}
	...
	public function add_my_stylesheet() {
		...
		$this->enqueue_local_stylesheet('vrp_submitreview','css/vrp.submitReview.css');
	}

	private function enqueue_local_stylesheet( $handle, $file ) {
		if ( file_exists( get_stylesheet_directory() . '/vrp/' . $file ) ) {
			wp_enqueue_style(
				$handle,
				get_stylesheet_directory_uri() . '/vrp/' . $file
